...
export class PrismaCategoryRepository implements ICategoryRepository {
  create(data: { name: string; slug: string; parentId?: number|null; position?: number }): Promise<Category> {
    return prisma.category.create({ data: {
      name: data.name, slug: data.slug, parentId: data.parentId ?? null, position: data.position ?? 0
    }});
  }
